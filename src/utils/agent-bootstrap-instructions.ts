...
export type BootstrapTarget = "claude-code" | "openclaw" | "opencode" | "cursor";

export function getBootstrapInstructions(target: BootstrapTarget): string {
  switch (target) {
    case "claude-code":
...
export function getBootstrapTargetLabel(target: BootstrapTarget): string {
  switch (target) {
    case "claude-code":
      return "CLAUDE.md";
    case "openclaw":
