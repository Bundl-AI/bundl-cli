...
function inferFromPackageJson(cwd: string): Partial<ProjectContext> {
  const path = resolve(cwd, "package.json");
  if (!existsSync(path)) return {};
  try {
    const raw = readFileSync(path, "utf-8");
    const pkg = JSON.parse(raw) as { description?: string; scripts?: Record<string, string>; dependencies?: Record<string, string> };
    ...
    for (const [roleSlug, hints] of Object.entries(ROLE_HINTS)) {
      ...
      if (matchCount >= 2) return { role: roleSlug, industry: "B2B SaaS", confidence: "high" };
      if (matchCount === 1) return { role: roleSlug, industry: null, confidence: "low" };
    }
  } catch {
    // ignore
  }
...
function inferFromClaudeMd(cwd: string): Partial<ProjectContext> {
  ...
  try {
    ...
    for (const [roleSlug, hints] of Object.entries(ROLE_HINTS)) {
      ...
      if (matchCount >= 2) return { role: roleSlug, industry: null, confidence: "high" };
      if (matchCount === 1) return { role: roleSlug, industry: null, confidence: "low" };
    }
  } catch {
    // ignore
...
function inferFromCrmFiles(cwd: string): Partial<ProjectContext> {
