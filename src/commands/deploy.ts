...
import type { CompilerOptions, CompilerResult } from "../targets/types.js";

const CORPUS_DIR = ".bundl/corpus";

const TARGETS = ["claude-code", "openclaw", "opencode", "cursor"] as const;
...
function loadCorpus(cwd: string): Corpus[] {
  const corpusPath = resolve(cwd, CORPUS_DIR);
  const files = readdirSync(corpusPath).filter(
...
function getCompilerOptions(cwd: string): CompilerOptions {
...
export async function runDeploy(options: DeployOptions = {}): Promise<number> {
  ...
  let files: string[];
  try {
    files = readdirSync(resolve(cwd, CORPUS_DIR)).filter(
      (f) => f.endsWith(".yaml") || f.endsWith(".yml")
    );
  } catch {
    files = [];
  ...
  if (hasErrors && !options.ci && !options.json) {
    for (const r of validation.results) {
      if (r.error) logger.error(`${r.file}: ${r.error}`);
    }
  ...
  const toRun: DeployTarget[] =
    target === "all" ? [...TARGETS] : [target as DeployTarget];
  ...
  if (options.ci) {
    for (const p of allWritten) logger.log(p);
    for (const w of allWarnings) logger.warn(w);
    return 0;
  }

  for (const r of results) {
    logger.success(`${r.target}: ${r.filesWritten.length} file(s) written`);
    for (const p of r.filesWritten) logger.log(`  ${p}`);
