...
import type { ProviderType } from "../utils/provider.js";
...
function loadSkills(cwd: string): Corpus[] {
...
function loadAllScenarios(cwd: string): { skillId: string; scenario: ScenarioFile }[] {
  const base = resolve(cwd, SCENARIOS_DIR);
  const out: { skillId: string; scenario: ScenarioFile }[] = [];
  try {
    ...
    for (const skillId of skillDirs) {
      ...
      for (const file of files) {
        ...
        const scenario = yaml.load(raw) as ScenarioFile;
...
function buildUserMessage(
  required: Record<string, string>,
  optionalProvided: Record<string, string>,
  optionalSkipped: { name: string; fallback: string }[]
): string {
...
function printOutputBox(skillName: string, output: string): void {
  console.log(DIV);
...
export async function runSimulate(options: SimulateOptions = {}): Promise<number> {
  ...
  if (ciMode && !allMode) {
    if (!jsonMode) logger.error("--ci requires --all");
    return 1;
  }
  ...
  if (allMode) {
    ...
    const results: { scenario_name: string; skill_id: string; passing: boolean; explanation?: string }[] = [];
    for (const { skillId, scenario } of scenarios) {
      ...
      const userMessage = buildUserMessage(
        ...
        scenario.inputs.optional_provided ?? {},
        optionalFallbacksForRun
      );
      const output = await callProvider(provider, systemPrompt, userMessage);
      const graderResponse = await runGrader(
        provider,
    ...
    if (ciMode) {
      for (const r of results) {
        if (r.passing) logger.log(`✓ ${r.scenario_name}`);
        else logger.error(`${r.scenario_name} — ${r.explanation ?? "behavior changed"}`);
      }
      logger.log("");
    ...
    for (const r of results) {
      if (r.passing) logger.success(`${r.scenario_name} — behavior matches`);
      else {
  ...
  if (workflowId) {
    ...
    const choice = await inquirer.prompt<{ skill: string }>([
      {
        type: "list",
        name: "skill",
  ...
  if (generateScenarios && !jsonMode) {
    ...
    const { selected } = await inquirer.prompt<{ selected: string[] }>([
      { type: "checkbox", name: "selected", message: "Select scenarios to run and optionally save", choices: choices as { name: string; value: string }[] },
    ]);
    ...
    for (const block of selectedBlocks) {
      ...
      for (const line of lines.slice(1)) {
        ...
        if ((skill.inputs.required ?? []).some((i) => i.name === name)) req[name] = value;
        else if ((skill.inputs.optional ?? []).some((i) => i.name === name)) opt[name] = value;
      }
      ...
      const { saveThis } = await inquirer.prompt<{ saveThis: boolean }>([
  ...
  for (const input of skill.inputs.optional ?? []) {
    ...
    if (a.value.trim()) optionalProvided[input.name] = a.value.trim();
    else optionalSkipped.push(input.name);
  }
  ...
  if (!jsonMode) {
    ...
    logger.log(
      `Optional inputs: ${Object.keys(optionalProvided).length} / ${(skill.inputs.optional ?? []).length}`
    );
  ...
  if (save) {
    ...
    const { scenarioName } = await inquirer.prompt<{ scenarioName: string }>([
    ...
    const { expectedBehavior } = await inquirer.prompt<{ expectedBehavior: string }>([
      {
        type: "input",
