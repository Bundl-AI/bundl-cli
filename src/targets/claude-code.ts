...
function communicationStyleSkill(skills: Corpus[]): Corpus | null {
  return (
    skills.find(
      (s) =>
        s.category === "communication-style" ||
        s.id.toLowerCase().includes("communication-style")
    ) ?? null
  );
}
