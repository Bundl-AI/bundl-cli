...
export function runSemanticChecks(parsed) {
    const issues = [];
    const names = inputNames(parsed);
    const suggest = (msg, sug) => issues.push({ severity: "error", message: msg, suggestion: sug });
    const warn = (msg, sug, lineHint) => issues.push({ severity: "warning", message: msg, suggestion: sug, lineHint });
    const varsInPrompt = extractVariables(parsed.system_prompt);
    for (const v of varsInPrompt) {
        if (!names.has(v)) {
            suggest(`Variable {{${v}}} used in system_prompt but not declared in inputs.required or inputs.optional`, `Add a required or optional input with name: "${v}" (and description, example, source).`);
        }
    }
    if (parsed.surfaces.agent && (!parsed.success_criteria || parsed.success_criteria.length === 0)) {
        suggest("success_criteria is empty — agent will not know when this workflow is complete", 'Add at least one success criterion, e.g. "crm_record_updated" or "email_sent".');
    }
    if (parsed.load === "on-demand") {
        const td = parsed.trigger_description?.trim() ?? "";
        if (td.length < 20) {
            suggest("load is on-demand but trigger_description is missing or under 20 characters", "Write a specific trigger_description (20+ chars) so the agent knows when to load this skill.");
        }
    }
    if (parsed.type === "document") {
        const ex = parsed.example_output?.trim() ?? "";
        if (ex.length < 100) {
            suggest("type is document but example_output is missing or under 100 characters", "Add a realistic example_output (100+ chars) so the document format is clear.");
        }
    }
    const tools = parsed.tools ?? [];
    if ((tools.includes("crm_write") || tools.includes("email_send")) &&
        (!parsed.handoff?.conditions || parsed.handoff.conditions.length === 0)) {
        suggest("Skills with crm_write or email_send must define handoff.conditions", "Add at least one condition under handoff.conditions and set escalate_to and handoff_message.");
    }
    for (const opt of parsed.inputs.optional ?? []) {
        if (opt.fallback === undefined || opt.fallback === "") {
            warn(`optional input '${opt.name}' missing fallback field`, `Add fallback: "Not provided — use default behavior" or a specific default.`, "inputs.optional");
        }
    }
    if (parsed.surfaces.agent && (!parsed.constraints || parsed.constraints.length === 0)) {
        warn("constraints array is empty on an agent-surface skill", "Add at least one constraint to guide agent behavior.");
    }
    const exampleLen = (parsed.example_output ?? "").trim().length;
    if (exampleLen > 0 && exampleLen < 200) {
        warn("example_output under 200 chars (too short to be useful)", "Expand example_output to a realistic 200+ character example.");
    }
    const triggerLen = (parsed.trigger_description ?? "").trim().length;
    if (triggerLen > 200) {
        warn("trigger_description over 200 chars (too long, agent may not use it)", "Shorten trigger_description to under 200 characters.");
    }
    const ver = (parsed.version ?? "").trim();
    if (ver && !SEMVER_REGEX.test(ver)) {
        warn("version is not semver format", 'Use semver, e.g. "1.0.0" or "1.2.3-beta.1".');
    }
    return issues;
}
function applyFixes(parsed) {
    let changed = false;
...
export async function runValidate(options = {}) {
    ...
    if (options.file) {
        ...
        if (existsSync(resolve(corpusPath, withYaml)))
            files = [withYaml];
        else if (existsSync(resolve(corpusPath, withYml)))
            files = [withYml];
        else if (existsSync(inCorpus))
            files = [options.file];
        else {
    ...
    for (const file of files) {
        ...
        const status = finalErrors > 0 ? "error" : finalWarnings > 0 ? "warning" : "valid";
    ...
    try {
        ...
        writeFileSync(lastValidatePath, JSON.stringify({ at: new Date().toISOString(), agentReady: agentReadyCount, total: results.length }), "utf-8");
    }
    catch {
        // ignore
    }
