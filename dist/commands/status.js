...
function getLastEditedCorpusFile(cwd) {
...
function getLastDeployTime(cwd) {
...
function getPendingChangeCount(cwd) {
    const corpusPath = resolve(cwd, CORPUS_DIR);
    if (!existsSync(corpusPath))
        return 0;
    ...
    let count = 0;
    for (const f of files) {
        if (statSync(resolve(corpusPath, f)).mtimeMs > lastDeploy)
            count++;
    }
    return count;
}
function loadSkillsAndValidate(cwd) {
    const corpusPath = resolve(cwd, CORPUS_DIR);
    let paths = [];
    try {
...
function loadScenariosStats(cwd) {
...
function loadLastValidated(cwd) {
    try {
